...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-orange-50 via-amber-50 to-white text-orange-950">
      <header className="sticky top-0 z-20 border-b border-orange-100 bg-white/80 backdrop-blur-md">
        <div className="mx-auto flex max-w-6xl items-center gap-4 px-4 py-3">
          <div className="flex items-center gap-2">
            <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-orange-500 text-white font-extrabold shadow-md shadow-orange-200">
              PP
            </div>
            <div>
              <div className="text-lg font-bold">Pangpaan POS</div>
              <div className="text-xs text-orange-700">Warm & fast iPad-friendly UI</div>
            </div>
          </div>
          <nav className="ml-auto hidden items-center gap-2 rounded-full bg-orange-50 px-2 py-1 text-sm shadow-inner shadow-orange-100 md:flex">
            {navItems.map((item) => {
              ...
              return (
                <Link
                  ...
                  className={cn(
                    ...
                    active ? "bg-white text-orange-900 shadow-sm shadow-orange-100" : "text-orange-700 hover:bg-white/70",
                  )}
                >
                  <Icon size={16} />
                  {item.label}
      ...
      <footer className="border-t border-orange-100 bg-white/60 py-3 text-center text-xs text-orange-700">
