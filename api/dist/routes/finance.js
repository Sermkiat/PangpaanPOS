...
router.get("/waste", asyncHandler(async (_req, res) => {
    const rows = await db.query.wasteRecords.findMany({
        orderBy: (tbl, { desc }) => desc(tbl.recordedOn),
    });
    return ok(res, rows);
}));
router.post("/waste", asyncHandler(async (req, res) => {
...
router.get("/allocation-rules", asyncHandler(async (_req, res) => {
    const rows = await db.query.allocationRules.findMany();
    return ok(res, rows);
}));
router.post("/allocation-rules", asyncHandler(async (req, res) => {
...
router.put("/allocation-rules/:id", asyncHandler(async (req, res) => {
